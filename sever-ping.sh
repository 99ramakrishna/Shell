...
do
 case $option in
   ...
   ;;
   54.86.37.79_connect )
        ssh -i  ansible-control.pem  -o StrictHostKeyChecking=No -n ubuntu@54.86.37.79
   ;;
   52.207.10.109_connect )
        ssh -i  ansible-control.pem  -o StrictHostKeyChecking=No -n centos@52.207.10.109
   ;;
   ping_servers )
   ...
   disk_spaces )
        echo -e "Enter ips text file name with extension example.txt :  \c"
        read  filename
        while read  centos 52.207.10109
        do
           user_ip=($item)
           user=${user_ip[0]}
           ip=${user_ip[1]}
           echo  "==================    $ip  =============================="
           echo " "
           storage=($(ssh -i  ansible-control.pem  -o StrictHostKeyChecking=No -n $user@$ip  "df -kh  | grep /dev/shm"))
